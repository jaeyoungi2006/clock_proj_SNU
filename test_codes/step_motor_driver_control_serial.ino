...
void StepMotor::calc(){
  ...
  if(time > time_move && is_rotating){
    ...
    if(tick_from_rot >= tick_target){
      Stop();
      Serial.println((String) "Moved to " + target_num);
...
void StepMotor::Start(int target = -1){
  ...
  if(target != -1){
    ...
    if(need_move == 0) is_rotating = 0;
    else if(need_move < 0) need_move += 10;
    tick_target = stride_num / 10 * need_move; //how much need to move(in tick)
    target_num = target;
  }
  else{
    tick_target = -1;
  }
  if(is_rotating == 1) Serial.println("Moving");
}
void StepMotor::Stop(){
  is_rotating = 0;
}
...
void loop() {
  ...
  if(check % 10 == 0 && Serial.available()){
